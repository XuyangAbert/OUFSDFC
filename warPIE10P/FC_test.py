...
def Input():
    ...
    sample = pd.read_csv('PIE100.csv',header=None)
...
if __name__ == '__main__':
    ...
    for i in range(NumofBatches):
        ...
        if num_hist == 0:
            ...
            concate_features = np.concatenate([data[:, histfidx], Features], axis = 1)
        ...
        # N_F = np.shape(Features)[1]
        # [DC_means, DC_std] = Distribution_Est(histsummary, Features, N_F)
        # [DisC, Dist] =  Feature_Dist(histsummary, Features, N_F)
        ...
        [PeakIndices, Pfitness, C_Indices] = Pseduo_Peaks(disc_curr, dist_curr, curr_mean, curr_std, Features, fitness_curr,
                                                          StdF, gamma)
        # [PeakIndices, Pfitness, C_Indices] = Pseduo_Peaks(DisC, Dist, DC_means, DC_std, Features, fitness,
        #                                                   StdF, gamma)
        fitness = oldfitness

        # Pseduo Clusters Infomormation Extraction
        PseDuo = curr_mean[PeakIndices]  # Pseduo Feature Cluster centers
        ...
        # if len(PseDuo) > 1:
        #     [FCluster, Ffitness, C_Indices] = Pseduo_Evolve(DisC, PeakIndices, PseDuoF, C_Indices, DC_means,
        #                                                     DC_std, Features, fitness, StdF, gamma)
        # else:
        #     FCluster = PseDuo
        #     Ffitness = PseDuoF
        #-----------Check for possible merges among historical feature clusters and current feature clusters----------#
            ...
            for k in range(len(EvolvedFC)):
                if EvolvedFC[k] in histFcluster:
                    ...
                    if len(EvolvedF) == 0:
                        ...
                        EvolvedF = np.append(EvolvedF, fitness[histidx])
                else:
        ...
        #     Extract_FIndices = SF
        # else:
        #     Extract_FIndices = np.append(Extract_FIndices, SF)
        # current_summary = np.transpose(np.asarray([SF, Ffitness]))
        SF = EvolvedFC
        SFfit = EvolvedF
    ...
    np.savetxt('Extract_Mad2.txt', Extract_Data)
    end = time.time()
