...
def Input():
    ...
    sample = pd.read_csv('lymphoma.csv',header=None)
...
def hist_merge(DisC, currentFcluster, currentFfitness, C_Indices, histFcluster, histFfitness, t, Batchsize):
...
#    group_kfold = KFold(n_splits=10,shuffle=True)
    ...
    iterations = 1
    acc_hist, f1_hist, sf_hist = [], [], []
    acc_cross, f1_cross = [], []
    sf_cross = []
    for it in range(iterations):
        
        for train_idx, test_idx in group_kfold.split(data,label):
#        for train_idx, test_idx in group_kfold.split(data):
            ...
            histsummary = []
            histStdF = []
#            f_rel = mutual_info_classif(train_data,train_label)
...
#                for k in range(len(FCluster)):
#                    temp_idx = np.where(C_Indices == FCluster[k])[0]
#                    temp_rel = f_rel[temp_idx+i*Batchsize]
#                    max_idx = np.argmax(temp_rel)
#                    FCluster[k] = temp_idx[max_idx]
                ...
                if i > 0:
                    ...
                    EvolvedFC = hist_merge(DisC, FCluster2, Ffitness, C_Indices2, histFcluster, histFfitness, i, Batchsize)
                    EvolvedF = []
                ...
                # print(np.shape(current_summary))
...
#            clf  = tree.DecisionTreeClassifier()
            clf = clf.fit(train_data[:, Extract_FIndices], train_label)
            predict_label = clf.predict(test_data[:, Extract_FIndices])
            
            accuracy = accuracy_score(test_label, predict_label)
            f1 = f1_score(test_label, predict_label,average='macro')
            acc_cross.append(accuracy)
            f1_cross.append(f1)
            sf_cross.append(len(SF))
        acc_hist.append(np.mean(acc_cross))
        f1_hist.append(np.mean(f1_cross)) 
        sf_hist.append(np.mean(sf_cross))
    print('10 runs average of 10-fold cross validated accuracy: ', np.mean(acc_hist))
    print('10 runs std of 10-fold cross validated accuracy: ', np.std(acc_hist))
    print('10 runs average of 10-fold cross validated f1: ', np.mean(f1_hist))
    print('10 runs std of 10-fold cross validated f1: ', np.std(f1_hist))
    print('10 runs average of 10-fold cross validated time: ', np.mean(sf_hist))
        
        
##    label = label.reshape(N, 1)
##    Extract_Data = np.concatenate((data[:, Extract_FIndices],label),axis=1)
##    np.savetxt('Extract_Mad3.txt', Extract_Data)
#    end = time.time()
#    print('The total time in seconds:', end-start)
#    print('The final selected feature subset: ', len(Extract_FIndices))
